...
The standard abstract model is consider in this work. The system consist of set of asynchronous unreliable processes $P=\{p_1,\ldots,p_n\}$ that communicate by applying operations on shared \emph{base objects} that support atomic operations such as reads, writes, and read-modify-write primitives. 

We assume that a once a process crash, the system knows at which line of the code the fail happened. However, there is no indication whether the instruction at that line tool affect before the crash. Upon recovering, the controller moves the process to a recovery code according to the line where the crash occur. By running this code, the process is aware that a crash happened, as well as at what line of the code was it. 
